...
if __name__ == '__main__':
    ...
    if rank == 0:  # el maestro comprueba que se ingresa la cantidad de paramentros correspondientes para trabajar
        ...
        try:
            ...
            for i in range(1, size):
                comm.send(True, dest=i)  # si todo esta en orden enviamos una senal de True, que significa que los nodos
                #                                                          seguiran trabajando
    ...
    if rank != 0:
        ...
        if trabajar != True:
            sys.exit(2)  # si el maestro no nos pasa un True, terminamos la ejecucion
    ...
    if rank == 0:
        for i in range(1, size):
            if i > 1:
                constructImg = np.concatenate(
                    (constructImg, comm.recv(source=i)))  # el maestro recibe las imagenes y las arma en una
            if i == 1:
                constructImg = comm.recv(source=i)
        imgContructFinal = Image.fromarray(constructImg)
        if (int(alfa * 10) == 10):
            imgContructFinal.save(os.getcwd() + "/images/movimiento/suma/OUTPUT_0.jpg")  # archivo de salida.
            imgContructFinal.save(os.getcwd() + "/images/movimiento/suma/OUTPUT0" + str(21 - int(alfa * 10)) + ".jpg")
        else:
            imgContructFinal.save(os.getcwd() + "/images/movimiento/suma/OUTPUT_1.jpg")  # archivo de salida.
            imgContructFinal.save(os.getcwd() + "/images/movimiento/suma/OUTPUT0" + str(21 - int(alfa * 10)) + ".jpg")
