...
def nearest_neighbor(arrImg, nuevo_ancho, nuevo_largo): #entra por parametro una imagen (PNG, JPG, etc) y las nuevas medidas de las imagenes
...
def nearest_paralelo(imagen, ancho,largo): #ENTRA UNA IMAGEN
    ...
    if rank==0 :
        ...
        imgContrucFinal.save("REDIMENCION_NEAREST.jpg") #GUARDAMOS LA IMAGEN
    ...
    if rank==0 :
        for i in range(1,size):
            ...
            if i == 1:
                construcImg = comm.recv(source=i)
        imgContrucFinal=Image.fromarray(construcImg)
        imgContrucFinal.save(os.getcwd() + "/images/REDIMENCION_BILINEAL.jpg") #GUARDAMOS LA IMAGEN
...
bilineal_paralelo(imagen,argv[1],argv[1]) # ----- > AQUI VAMOS VARIANDO LA FUNCION Y LOS TAMAÑOS
